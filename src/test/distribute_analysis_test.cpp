...
int main(int argc, char *argv[]) {
    ...
    int K = 20;
    std::string dataset_path = "/home/zzlin/dataset/gist/train_600000/gist_base.fvecs";
    ...
    std::string index_path = "/home/zzlin/dataset/gist/train_600000/hnsw_gist_M16_efConstruction500_operator0.index";
    int op = 5;
    ...
    if (op >= 2) {
        ...
        if (op == 5) {
            ...
            ddcres->base_square_.resize(data_list.size());
            for (int i = 0; i < data_list.size(); i++) {
                float square = 0.0;
                ...
                if (i % 50000 == 0)
                    std::cerr << "Compute base square progress: " << i << " / " << data_list.size()
                              << std::endl;
            }
        }

...
for op in {0..6}; do
  for t in "${init[@]}"; do
    ...
    log_file="${ORIGINAL_DIR}/delta/hnsw_gist_K${K}_operator${op}_search_init${t}_delta${delta}_log.txt"

    ./hnsw_search -k "${K}" -n "${dataset_path}" -q "${query_path}" -g "${ground_truth_path}" -i "${index_path}" -o "${op}" -t "${t}" -d "${delta}" >"${log_file}" 2>&1

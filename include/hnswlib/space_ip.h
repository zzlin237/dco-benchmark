...
class InnerProductSpace : public SpaceInterface<float> {
 ...
 public:
    ...
    DISTFUNC<float> get_dist_func_by_dim(size_t dim) {
        ...
        if (dim % 16 == 0)
            distfunc = InnerProductSIMD16ExtSSE;
        else if (dim % 4 == 0)
            distfunc = InnerProductSIMD4ExtSSE;
        else if (dim > 16)
            distfunc = InnerProductSIMD16ExtResiduals;
        else if (dim > 4)
            distfunc = InnerProductSIMD4ExtResiduals;
#endif
        return distfunc;
    }

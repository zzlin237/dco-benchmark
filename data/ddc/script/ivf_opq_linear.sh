...
index_path="/home/zzlin/dataset/${dataset}/ivf_Q${dataset}_C4096_operator0.index"
output_path="/home/zzlin/dataset/${dataset}/ddc/${dataset}_ivf_opq_${K}_training_set.fvecs"
linear_path="/home/zzlin/dataset/${dataset}/ddc/linear_${type}${K}_${method}.txt"
log_file="${ORIGINAL_DIR}/log/ivf_${dataset}_K${K}_logger_hnsw_opq1.txt"

./logger_ivf_opq -k ${K} -n ${dataset_path} -q ${query_path} -g ${ground_truth_path} -c ${codebook_path} -i ${index_path} -o ${output_path} -r ${recall}  -l ${linear_path} > ${log_file} 2>&1
...
log_file="${ORIGINAL_DIR}/log/ivf_${dataset}_ddc_opq_linear_${K}.txt"
...
log_file="${ORIGINAL_DIR}/log/ivf_${dataset}_K${K}_logger_hnsw_opq2.txt"

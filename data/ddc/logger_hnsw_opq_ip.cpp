...
int main(int argc, char **argv) {
    ...
    std::unique_ptr<DDCopqIP> DCO = std::make_unique<DDCopqIP>(dataset_path, dim, false);
    count_bound = std::min(count_bound, (size_t) query_list.size());
    for (auto &query: query_list) {
        DCO->query_transform(query.data.data());
    }
    alg_hnsw->encoder_origin_data_ip(DCO.get());
    ...
    if (f.good()) {
        ...
        DCO->binary_search_multi_linear_ip(acc.size(), app.data(), acc.data(), thresh.data(), cluster.data(), count_base, recall);

...
int main(int argc, char **argv) {
    ...
    const struct option opts[] = {
            ...
            {"new_index_path",         required_argument, 0, 'e'},
    };
    ...
    while (iarg != -1) {
        iarg = getopt_long(argc, argv, "k:n:i:q:g:c:r:o:l:e:", opts, &ind);
        switch (iarg) {
            case 'k':
            ...
            case 'l':
                ...
                break;
            case 'e':
                if (optarg) new_index_path = optarg;
                break;
        }
    }
    ...
    if (new_index_path.empty()) {
        std::cerr << "Error: new_index_path is empty. Please provide a valid path using the -e option." << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    std::cout << "new_index_path: " << new_index_path << std::endl;

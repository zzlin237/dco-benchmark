...
for data in {0..1}; do
  for op in {0..7}; do
    ...
    log_file="${ORIGINAL_DIR}/log/ivf_cuda_${datasets[$data]}_K${K}_operator${rotate[$op]}_search_log.txt"
    ./ivf_cuda_search -k "${K}" -n "${dataset_path}" -q "${query_path}" -g "${ground_truth_path}" -i "${index_path}" -o "${rotate[$op]}" > "${log_file}" 2>&1

    if [ $? -ne 0 ]; then
        echo "${datasets[$data]} GPU Search FAIL"
        exit 1
    fi
  done
done

//
// Created by 郑荘霖 on 2025/7/12.
//

#ifndef DCOS_DDCOPQ_H
#define DCOS_DDCOPQ_H
...
class DDCopq : public DCO {
public:
    DDCopq(std::string dataset, size_t dim, std::string type = "hnsw")
            : DDCopq(dataset, dim, true, type) {
    }

    DDCopq(std::string dataset, size_t dim, bool load_model, std::string type = "hnsw")
            : space_(dim),
              dim_(dim) {

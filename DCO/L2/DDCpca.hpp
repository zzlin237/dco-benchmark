...
#ifndef DCOS_DDCPCA_H
#define DCOS_DDCPCA_H
...
class DDCpca : public DCO {
public:
    DDCpca(std::string dataset, size_t dim, std::string type = "hnsw")
            : DDCpca(dataset, dim, true, type) {
    }

    DDCpca(std::string dataset, size_t dim, bool load_model, std::string type = "hnsw")
            : space_(dim),
              dim_(dim),
              dim_delta_(delta_d),
